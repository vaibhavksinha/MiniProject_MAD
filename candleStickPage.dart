...
class _CandleStickState extends State<CandleStick> {
  ...
  GetData({required String name}) async {
    ...
    for (int i = 0; i < TimeStamp.length-1; i++) {
      ...
      candle.add(Candle(
        date: date,
        high: high[i]== null? high[i-1]:high[i],
        low: low[i]== null? low[i-1]:low[i],
        open: open[i]== null? open[i-1]:open[i],
        close: close[i]== null? close[i-1]:close[i],
        volume: 0.1,
      ));
    }
    setState(() {
    ...
    print(candle);print(widget.Loading);
  }
